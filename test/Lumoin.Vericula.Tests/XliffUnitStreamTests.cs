...
public sealed class XliffUnitStreamTests
{
    ...
    /// <summary>Flattens groups in document order within a bounded synchronous walk.</summary>
    [TestMethod]
    public async Task StreamsUnitsInDocumentOrderFlatteningGroups()
    {
        ...
        Task<string[]> enumerate = Task.Run(() => XliffReader.ReadUnits(stream).Select(unit => unit.Id).ToArray(), TestContext.CancellationToken);
        string[] ids;
        try
        {
            ids = await enumerate.WaitAsync(deadline.Token);
        }
        ...
        {
            Assert.Fail("Streaming units did not finish in document order within ten seconds.");
            throw;
        }
    ...
    /// <summary>Streams pipe units in order and reports a named failure if the walk stops advancing.</summary>
    [TestMethod]
    ...
    /// <summary>Refuses malformed XML before the first unit within a bounded synchronous walk.</summary>
    [TestMethod]
    public async Task StreamingRejectsMalformedXmlTruncatedBeforeAnyUnit()
    {
        ...
        //The swallowed XML error leaves a spinning worker, which this deadline abandons after
        //an explicit named failure without weakening the exact exception assertion.
        using var deadline = CancellationTokenSource.CreateLinkedTokenSource(TestContext.CancellationToken);
        deadline.CancelAfter(TimeSpan.FromSeconds(10));
        Task enumerate = Task.Run(() =>
            Assert.ThrowsExactly<XliffFormatException>(() => XliffReader.ReadUnits(stream).ToArray()), TestContext.CancellationToken);
        try
        {
            await enumerate.WaitAsync(deadline.Token);
        }
        catch(OperationCanceledException) when(deadline.IsCancellationRequested && !TestContext.CancellationToken.IsCancellationRequested)
        {
            Assert.Fail("Malformed XML before the first unit was not refused within ten seconds.");
        }
    }

    /// <summary>Refuses malformed XML before the first asynchronous unit within ten seconds.</summary>
    [TestMethod]
    public async Task StreamingAsyncRejectsMalformedXmlTruncatedBeforeAnyUnit()
    {
        ...
        //Consume the malformed document while observing the local deadline.
        ...
        //T-008 swallows the XML error; cancellation must become this named assertion failure.
        try
        {
            await Task.WhenAny(enumerateTask, Task.Delay(Timeout.InfiniteTimeSpan, deadline.Token));
            if(deadline.IsCancellationRequested && !TestContext.CancellationToken.IsCancellationRequested)
            {
                Assert.Fail("Malformed XML before the first async unit was not refused within ten seconds.");
            }
    ...
    /// <summary>Preserves file validation boundaries during bounded asynchronous enumeration.</summary>
    [TestMethod]
