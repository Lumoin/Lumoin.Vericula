...
- `XliffWriter` refuses, before writing anything, an inline part whose `Form` is not a defined `SpanForm` or `AnnotationForm` value, a part of a kind it cannot serialize, and inline codes or annotations nested more than 64 levels deep; before, such models reached the serializer's internal guards or, for very deep nesting, the process stack.
- `XliffReader`'s streaming walk no longer treats a self-closing `<group/>` as still open, which made it stop counting the members that followed it, refuse validation rules that followed it at file level, and refuse the next `<file>` as empty.
- `XliffReader` refuses a `<file>` nested inside another `<file>` or a `<group>` outside `<unit>` subtrees on both the whole-document and streaming paths (XLIFF 2.1 §4.2.2.1); before, the whole-document path silently dropped the nested file's units and the streaming path miscounted the outer file's members. The source generator reports the same refusal as `VFX305`.
