...
public static partial class XliffReader
{
    ...
    /// accepts it anywhere among the unit's children. A <c>&lt;data&gt;</c> entry that no code ends up
    /// referencing is simply never attached to a part and so is dropped, with no separate filtering step.
    /// </summary>
    ...
    /// <exception cref="XliffFormatException">If a <c>&lt;data&gt;</c> id is missing, malformed or duplicated, or an unqualified attribute is unsupported.</exception>
    private static ImmutableDictionary<string, OriginalData> ParseOriginalData(XElement unitElement, string unitId)
    {
        XElement? originalDataElement = unitElement.Element(Core + WellKnownXliffElements.OriginalData);
    ...
    /// any order). Any other child is ignored, matching how comments and processing instructions are
    /// ignored inside <c>&lt;source&gt;</c>/<c>&lt;target&gt;</c>.
    /// </summary>
    ...
    /// refused by the length check; a value with a non-hexadecimal character is refused separately so
    /// the two failure modes carry distinct messages.
    /// </summary>
    ...
    /// <see cref="WellKnownXliffAttributeValues.Generic"/>), <c>translate</c>, <c>ref</c> and
    /// <c>value</c>, refusing a <c>comment</c> annotation that carries neither <c>value</c> nor
    /// <c>ref</c> (XLIFF 2.1 §4.7.3.1.3).
    /// </summary>
    ...
    /// an id already used on this side, or (only on the target side) already used somewhere else in the
    /// unit and not the sibling source id this same segment introduced, is refused; otherwise the id is
    /// recorded (a legitimate reuse of the sibling source id is not recorded again, it is already there).
    /// </summary>
    ...
    {
        bool usedElsewhere = state.Ids.Contains(id) || (context.IsTarget && context.OtherSideIds.Contains(id));
        bool exempt = context.IsTarget && context.SiblingSourceIds.Contains(id);
        if(usedElsewhere && !exempt)
        {
        ...
        return state.Ids.Contains(id) ? state : state with { Ids = state.Ids.Add(id) };
    }
    ...
    /// <param name="OtherSideIds">The ids already used on the other side of the unit so far, consulted only when <see cref="IsTarget"/> is true.</param>
